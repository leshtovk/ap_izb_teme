...
check$summarize <- function() {
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4N30:1j6CyK:zSoASFsysVpfL2Zm8Y6swJyEA5k'
    tryCatch({
      check$equal(vsota.kvadratov(3), 14)
      check$equal(vsota.kvadratov(4), 30)
      check$equal(vsota.kvadratov(5), 55)
      for (i in 1 : 40) {
        check$secret(vsota.kvadratov(i), paste0("Koliko je vsota kvadratov do ", i, "?"))
      }
    },
    error = function(e) {
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4OH0:1j6CyK:K-jHLppKV8LROdFI2kCpHR--_zc'
    tryCatch({
