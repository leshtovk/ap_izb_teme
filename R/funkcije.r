...
velikost.slike <- function(diagonala, razmerje=16/9) {
  y <- sqrt(diagonala^2/(1 + razmerje^2))
  x <- razmerje * y
  y <- round(y, digits=0)
  x <- round(x, digits=0)
...
check$summarize <- function() {
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU3OX0:1j5zOd:3PWZ9LxF9YKRSc5_O5GlmH6h2t4'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4MH0:1j5zOd:qmjOKUTpvrtx7vNcZgYTA4su5f8'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4MX0:1j5zOd:XB5hiAbiZ9xedhLdEkp02yW0Lrc'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4Mn0:1j5zOd:0-MoOMjv-LpY_WD_yoPa7dzr4Rg'
    tryCatch({
  ...
  if (check$part()) {
    check$parts[[check$part.counter]]$token <- 'eyJ1c2VyIjoyNjkwLCJwYXJ0IjoyMTU4M30:1j5zOd:QCNSlqxsoNy1_XbIB9pHvBg6C-w'
    tryCatch({
