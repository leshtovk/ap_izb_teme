...
# how many years has the amount of men been greater than the amount of women

d <- dim(residents)
men.numbers <- residents$prebivalstvo[seq(1, d[1], by = 2)]
women.numbers <- residents$prebivalstvo[seq(2, d[1], by = 2)]
bools <- men.numbers > women.numbers
  
more.men <- sum(seq(1, 1, length = d[1]/2)[bools])

# what are the 20 most common names of 2019 

most.common <- filter(imena, leto == 2019) %>% group_by(ime) %>%
  summarise(stevilo=sum(stevilo)) %>% arrange(desc(stevilo)) %>%
  head(20)

# write `imena` on the disk as a `csv` file
# write.csv(imena, 'imena.csv')